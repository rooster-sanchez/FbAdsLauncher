...
- **PAC creative limits — one ad_format and one asset per label per rule.** Two related v25.0 errors when building flex creatives with `asset_customization_rules`:
  - Error subcode **1885374** ("a parts list can only have one ad format"): a single creative cannot have both `SINGLE_IMAGE` and `SINGLE_VIDEO` in `ad_formats`. Split into TWO creatives (statics, videos).
  - Error subcode **1885878** ("cannot apply multiple `images` pieces to rule N"): each `asset_customization_rules` entry binds ONE label, and that label can match only ONE asset in the creative. So a creative cannot ship 3 different 1:1 images all labeled FEED — they collide on the FEED rule.
  - The natural shape Meta wants is: **one creative = one concept = one (FEED-asset, VERTICAL-asset, OTHER-asset) triple**. To launch N concepts, build N creatives. Multi-asset multi-aspect-ratio creatives belong in `is_dynamic_creative=true` ad sets (which only allow 1 ad each).
...
- **`asset_feed_spec.posts` ≠ `asset_feed_spec.images` for ad-set-flag purposes.** Posts-based flex (`asset_feed_spec.posts: [{post_id}, ...]`) is treated as standard creative and works in non-dynamic ad sets. Raw-image flex (`asset_feed_spec.images: [{hash}, ...]`) is treated as **dynamic creative** by Meta and requires `is_dynamic_creative=true` on the ad set — otherwise creation fails with subcode 1885998 ("No se puede crear el anuncio de contenido dinámico en un conjunto de anuncios de contenido no dinámico"). Each existing post can carry both 1:1 + 9:16 in one asset, sidestepping the 10-asset cap; raw images count each aspect ratio separately.

- **`is_dynamic_creative` is immutable post-creation.** Trying to flip it via `POST /{adset_id}` with `is_dynamic_creative=true` returns `{success: true}` but the field stays `false` on subsequent reads. To run a raw-image flex ad in an existing non-dynamic ad set you have two options: (a) create a NEW sibling ad set in the same campaign with the flag set at creation, or (b) publish the images as Page posts first and use `asset_feed_spec.posts`.

- **Flex/dynamic ads cap at 10 unique image+video assets per `asset_feed_spec`.** Submitting >10 returns subcode 1815983 ("Los anuncios de contenido dinámico pueden incluir un máximo de 10 imágenes y vídeos"). Each unique hash counts — so 6 concepts × 2 aspect ratios = 12 → over cap. Workarounds: (a) one aspect ratio only (Meta auto-resizes for other placements), (b) drop concepts to fit, (c) split into two creatives, (d) use `asset_feed_spec.posts` so each post counts as 1 asset and carries both ratios internally.

- **`degrees_of_freedom_spec.creative_features_spec.standard_enhancements` is deprecated as of v25.0.** Including the wrapper toggle (even with `enroll_status: OPT_OUT`) returns subcode 3858504 ("La opción para incluir el campo de mejoras estándar en el anuncio está obsoleta. Define funciones individuales en su lugar"). Either omit `degrees_of_freedom_spec` entirely (Meta defaults apply) or define individual feature flags. Older one-off launch scripts (e.g., `launch_ts_airfly_costcap_flex.py`) still ship the wrapper — strip it before reusing them as templates.
