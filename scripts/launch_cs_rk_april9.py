#!/usr/bin/env python3
"""
One-off launch script: Christian Sexuality — Raising Kids
Campaign: 04-01-26_MMS_ABO_TESTING
Date: 2026-04-09

1 ad set with 3 static PAC ads + 1 carousel ad
"""
import json
import sys
...
from meta_api import (
    create_ad, create_adset, upload_image,
    create_pac_creative, create_carousel_creative, delete_object,
)
...
ADSET_NAME = "04-01-26_MMS_ABO_TESTING_CUSTOM_IGCS+FBPAGE_FOLLOWERS"

# ─── Files ───────────────────────────────────────────────────────────────────

DL = Path.home() / "Downloads"

# Static ads: (ad_name, 1x1_file, 9x16_file)
STATIC_ADS = [
    ("RK_8VideoEpisodes_PAC",  DL / "Course%20Presenters.png",   DL / "Course%20Presenters-2.png"),
    ("RK_LearnToAnswer_PAC",   DL / "The%20_ONLY_.png",          DL / "The%20_ONLY_-2.png"),
    ("RK_TheOnlyCourse_PAC",   DL / "The%20_ONLY_%203.png",      DL / "The%20_ONLY_%205.png"),
]

# Carousel cards: (card_headline, 1x1_file)
CAROUSEL_CARDS = [
    ("Waiting until you feel ready",      DL / "Course%20Content%204.png"),
    ("Step in. Answer their questions.",   DL / "Course%20Content%205.png"),
    ("Expert-led course",                 DL / "Course%20Content%206.png"),
]
...
def main():
    print("=" * 60)
    print("CS Raising Kids — Launch Script")
    print("=" * 60)
    ...
    try:
        ...
        print("\n--- Uploading images ---")
        static_hashes = []  # [(label, feed_hash, vertical_hash)]
        ...
        carousel_hashes = []
        for card_name, fpath in CAROUSEL_CARDS:
            print(f"  Uploading carousel card: {card_name}")
            h = upload_image(config, str(fpath))
            carousel_hashes.append(h)

        total = len(static_hashes) * 2 + len(carousel_hashes)
        ...
        # ── Step 3: Create 3 static PAC ads ─────────────────────────────
        ...
        # ── Step 4: Create carousel ad ──────────────────────────────────
        print("\n--- Creating carousel ad ---")
        carousel_media = [{"type": "image", "ref": h} for h in carousel_hashes]
        carousel_creative_id = create_carousel_creative(
            config,
            name="RK_CourseContent_Carousel",
            media_refs=carousel_media,
            headline="Get Access",
            primary_text=PRIMARY_TEXT,
            description="",
            destination_url=LANDING_PAGE,
        ...
        created_objects.append(("creative", carousel_creative_id, True))

        carousel_ad_id = create_ad(
            config, "RK_CourseContent_Carousel", adset_id,
            carousel_creative_id, status="PAUSED",
        )
        created_objects.append(("ad", carousel_ad_id, True))
        print(f"  Carousel ad created: {carousel_ad_id}")
        ...
        print(f"Ads: 3 static PAC + 1 carousel = 4 ads total")
